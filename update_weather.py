...
def update_weather(api_key, city_entry, temp_label, time_label, heating_status_label, blinds_status_label, root, automation_enabled, light_statuses, light_buttons, light_status_label):
    ...
    if 'main' not in data:
        ...
        local_time = datetime.now(pytz.utc) + timedelta(seconds=timezone_offset)  # Add timezone offset to current time
        local_time_str = local_time.strftime('%Y-%m-%d %H:%M:%S')
        temperature = data['main']['temp']
        sunrise = datetime.fromtimestamp(data['sys']['sunrise'], pytz.utc) + timedelta(seconds=timezone_offset)
        sunset = datetime.fromtimestamp(data['sys']['sunset'], pytz.utc) + timedelta(seconds=timezone_offset)
